 ...
 *
 * Primary path  : Groq (openai/gpt-oss-120b), configured for this project.
...
export async function generateAITravelResponse(message, options = {}) {
  ...
  try {
    ...
    console.error('[SafarAI AI] Groq request failed:', error);
    throw new Error(
      error?.name === 'AbortError'
        ? 'The assistant took too long to respond. Please try again.'
        : error?.message || 'The assistant could not reach Groq. Please try again.'
    );
  }
}
